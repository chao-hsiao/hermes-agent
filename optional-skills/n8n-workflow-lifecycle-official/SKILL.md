...
| `references/UPDATE_PITFALLS.md` | About to run `update_workflow` with expression values or multiline content |

## Exhaustive scope when asked for "all"
When the user asks for "all .md files", "every workflow", or any exhaustive enumeration, do not stop at the first obvious category. Search the full target scope first, then summarize by category. Stopping early looks like incomplete research and triggers rework.
